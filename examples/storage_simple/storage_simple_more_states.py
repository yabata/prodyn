...
#we define a battery with 5 kWh capacity and define our steps to be 6 
#(0kWh, 1kWh, 2kWh, 3kWh, 4kWh, 5kWh)
states.loc['battery','xmin'] = 0
states.loc['battery','xmax'] = 5
states.loc['battery','xsteps'] = 6
...
#for 24 hours, so we define a numpy array [0,1,...,23]
...
timeseries['el. price'] = np.array(
                            [5,5,5,4,3,2,1,1,1,2,4,5,5,5,6,7,9,7,9,7,8,8,8,5])
...
result_fw = prd.DP_forward(states,controls,timesteps,constants,timeseries,storage_model,verbose=True,t_verbose=10)
...
#DP_backward
###
result_bw = prd.DP_backward(states,controls,timesteps,constants,timeseries,storage_model,verbose=True,t_verbose=10)
...
#And compare it to the results where the storage is full at the beginning (Xidx_start=5)
result5_bw = result_bw.xs(5,level='Xidx_start')
